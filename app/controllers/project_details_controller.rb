class ProjectDetailsController < ApplicationController
  before_action :set_project_detail, only: %i[ show edit update destroy ]

  # GET /project_details or /project_details.json
  def index
    @project_details = ProjectDetail.all
  end

  # GET /project_details/1 or /project_details/1.json
  def show
  end
  ...
  # POST /project_details or /project_details.json
  def create
    @project_detail = ProjectDetail.new(project_detail_params)

    respond_to do |format|
      if @project_detail.save
        format.html { redirect_to project_detail_url(@project_detail), notice: "Project detail was successfully created." }
  ...
  # PATCH/PUT /project_details/1 or /project_details/1.json
  def update
    respond_to do |format|
      if @project_detail.update(project_detail_params)
        format.html { redirect_to project_detail_url(@project_detail), notice: "Project detail was successfully updated." }
  ...
  # DELETE /project_details/1 or /project_details/1.json
  def destroy
  ...
  private
    ...
    def project_detail_params
      params.require(:project_detail).permit(:name)
    end
end
